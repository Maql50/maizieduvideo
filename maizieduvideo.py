...
class MaiZiVideo(object):
    ...
    def download_video(all_course_url, file_dir):
        ...
        try:
            ...
            print "已存在文件夹"
        all_course_url_and_name = re.findall(
            r'<li><a href="(.*?)" target="_blank" class="font14 color66"><span class="fl">(.*?)</span>', html_source,
            re.S)
        ...
        for each_course in all_course_url_and_name:
            ...
            html_source = response.text
            print html_source
            mp4_url = re.search(r'\$lessonUrl = "(.*?)"', html_source, re.S).group(1)
